...
// Helper components (bạn có thể tách ra file riêng)
...
const AdminBookingDetail = () => {
  ...
  const handleUpdateStatus = async (newStatus, newPaymentStatus) => {
    ...
    try {
      ...
      setBooking(response.data); // Cập nhật state với dữ liệu mới
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <h2 className="text-3xl font-bold mb-6">Chi tiết Booking #{booking.id}</h2>
         ...
         <div className="space-y-4">
            {booking.bookingDetails && booking.bookingDetails.map(detail => (
                <div key={detail.id} className="border border-gray-200 rounded-lg p-4">
                    <h4 className="font-semibold text-lg text-blue-700">{detail.room.name}</h4>
